...
where
    T: Send + Sync + Sub<Output = T> + Add<Output = T> + Mul<Output = T> + AddAssign + Copy + std::iter::Sum,
{
...
where
    T: Send + Sync + Sub<Output = T> + Add<Output = T> + Mul<Output = T> + Copy + AddAssign + std::iter::Sum,
...
where
    T: Copy + Send + Sync + Sub<Output = T> + Add<Output = T> + Mul<Output = T> + AddAssign + std::iter::Sum,
    U: UniqueIdentifier<DataType = Vec<T>>,
{
