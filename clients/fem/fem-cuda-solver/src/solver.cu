...
__global__ void step_kernel(mode_state_space *mss, double *v, double *y,
  ...
  if (i < n) {
    ...
    mss->x0 = mss->a0 * x0 + mss->a2 * x1 + mss->b2 * v[i];
    // x1 <- a1 x0 + a3 x1 + b3 vi
    mss->x1 = mss->a1 * x0 + mss->a3 * x1 + mss->b3 * v[i];
    y[i] = mss->x0;
  }
}
  void state_space::build(int n_mode, mode_state_space *mss, int n_input, double *i2m,
  ...
  void state_space::step(double *u, double *y) {
    ...
    // v = Bu
    cublasDgemv(handle, CUBLAS_OP_N, n_mode, n_input, &alpha, d_i2m, n_mode,
                d_u, 1, &beta, d_v, 1);
    ...
    cublasDgemv(handle, CUBLAS_OP_N, n_output, n_mode, &alpha, d_m2o, n_output,
                d_x0, 1, &beta, d_y, 1);
    cudaMemcpy(d_y, y, n_output * sizeof(double), cudaMemcpyDeviceToHost);
  }
